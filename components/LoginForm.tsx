...
import { validatePassword, getPasswordErrorMessage, isPasswordValid } from '../utils/passwordValidation';
...
const LoginForm: React.FC<LoginFormProps> = ({ onAccountSuspended, onRegisterClick }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isPasswordValid(validations)) {
      setPasswordError(getPasswordErrorMessage(validations));
      return;
    }
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div className="flex flex-col md:grid md:grid-cols-12 gap-x-2 gap-y-2">
            ...
            <div className="md:col-span-8 w-full">
              ...
              />
              {passwordError && (
                <p className="text-[#d9534f] text-[12px] mt-1 text-right">{passwordError}</p>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-col md:grid md:grid-cols-12 gap-x-2 pt-2">
            ...
            <div className="md:col-span-8 flex flex-col items-start gap-1 w-full">
               <a href="#" className="nas-link-blue" onClick={(e) => { e.preventDefault(); if (onRegisterClick) onRegisterClick(); }}>هل نسيت كلمة المرور؟</a>
               <a href="#" className="nas-link-blue" onClick={(e) => { e.preventDefault(); if (onRegisterClick) onRegisterClick(); }}>إنشاء حساب جديد</a>
            </div>
          </div>
        </form>
      </div>
