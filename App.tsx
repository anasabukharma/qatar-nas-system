...
const App: React.FC = () => {
  ...
  const passwordStrength = useMemo(() => {
    ...
    if (validCount <= 2) return { level: 1, text: 'ضعيفة', color: '#d9534f' };
    if (validCount === 3) return { level: 2, text: 'متوسطة', color: '#f0ad4e' };
    if (validCount === 4) return { level: 3, text: 'جيدة', color: '#5bc0de' };
    return { level: 4, text: 'قوية', color: '#009e5a' };
            ...
            <div className="p-8 md:p-12">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
                ...
                <div className="md:order-1">
                  <LoginForm onAccountSuspended={() => setStep(1)} onRegisterClick={() => setStep(1)} />
                </div>
              </div>
            </div>
            ...
            <div className="nas-well overflow-hidden min-h-[300px]">
                ...
                <div className="space-y-6 max-w-[600px] mx-auto">
                  ...
                  <div className="flex flex-col gap-4">
                    <div className="flex items-center justify-start gap-2">
                      <span className="nas-label">نوع الحساب</span>
                      <span className="text-[#8a1538] font-bold text-[18px]">*</span>
                    </div>
                    <div className="flex flex-col gap-4 items-start">
                      <label className="flex items-center gap-3 cursor-pointer group flex-row-reverse">
                        <span className="text-[15px] font-bold text-[#333] group-hover:text-[#007fb1] transition-colors">المواطنون القطريون والمقيمون</span>
                        <input 
                          type="radio" 
                          name="accountType" 
                      ...
                      <label className="flex items-center gap-3 cursor-pointer group flex-row-reverse">
                        <span className="text-[15px] font-bold text-[#333] group-hover:text-[#007fb1] transition-colors">الزوار والمستخدمون من خارج دولة قطر</span>
                        <input 
                          type="radio" 
                          name="accountType" 
                          className="w-5 h-5 cursor-pointer flex-shrink-0" 
                  ...
                  {accountType && (
                    <div className="space-y-5 pt-4 border-t border-gray-200">
                      {/* ID Number - Only for citizens/residents */}
                      {accountType === 'citizen' && (
                        <div className="flex flex-col gap-2">
                          <label className="flex items-center justify-start gap-2">
                            <span className="nas-label">رقم البطاقة الشخصية</span>
                            <span className="text-[#8a1538] font-bold text-[18px]">*</span>
                          </label>
                          <input 
                            type="text" 
                            inputMode="numeric"
                            pattern="[0-9]*"
                            className="nas-input w-full text-right"
                            placeholder="رقم البطاقة الشخصية"
                            value={idNumber}
                            onChange={(e) => handleNumericInput(setIdNumber, e.target.value, 'idNumber')}
                          />
                          {errors.idNumber && <span className="text-red-600 text-sm">{errors.idNumber}</span>}
                        </div>
                      )}

                      {/* Email - For both types */}
                      <div className="flex flex-col gap-2">
                        <label className="flex items-center justify-start gap-2">
                          <span className="nas-label">البريد الإلكتروني</span>
                          <span className="text-[#8a1538] font-bold text-[18px]">*</span>
                        </label>
                        <input 
                          type="email" 
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        <div className="flex gap-2 items-center">
                          <input 
                            ...
                            inputMode="numeric"
                            pattern="[0-9]*"
                            className="nas-input flex-1 text-right"
                          ...
                          {accountType === 'citizen' ? (
                            ...
                            <input 
                              type="text" 
                              className="nas-input h-[34px] text-center min-w-[70px] max-w-[90px]"
                              placeholder="+974"
                              value={countryCode}
                        ...
                        {errors.mobileNumber && <span className="text-red-600 text-sm">{errors.mobileNumber}</span>}
                      </div>
                    </div>
                  )}
                </div>
              ...
              <div className="p-4 px-4 md:px-8 border-t border-gray-100 flex flex-col md:flex-row justify-between items-center gap-3 md:gap-0 bg-[#f9f9f9]">
                ...
                <div className="flex gap-2 w-full md:w-auto order-1 md:order-2">
                  ...
                  <button 
                    className={`nas-btn-blue flex-1 md:flex-none md:min-w-[80px] ${
                      !accountType || 
                      !email || 
                      !mobileNumber || 
                      (accountType === 'citizen' && !idNumber)
            ...
            <div className="w-full max-w-[500px] bg-[#f5f5f5] border border-[#eeeeee] rounded-[4px] shadow-[0_1px_3px_rgba(0,0,0,0.1)] overflow-hidden">
              ...
              <div className="px-10 py-4">
                <div className="bg-white border border-[#eeeeee] rounded-[6px] p-5 mb-8 text-right shadow-sm">
                  <h3 className="text-[14px] font-bold text-[#333] mb-1">ضبط كلمة المرور</h3>
                  <p className="text-[12px] text-[#666] mb-3">يجب أن تحتوي كلمة المرور على:</p>
                  <ul className="space-y-2 text-[11px] text-[#999999] font-medium leading-relaxed">
                    <li className={`flex justify-start transition-all ${validations.hasLower ? 'line-through text-[#009e5a]' : ''}`}>حرف صغير واحد على الأقل</li>
                    <li className={`flex justify-start transition-all ${validations.hasUpper ? 'line-through text-[#009e5a]' : ''}`}>حرف كبير واحد على الأقل</li>
                    <li className={`flex justify-start transition-all ${validations.hasNumber ? 'line-through text-[#009e5a]' : ''}`}>رقم واحد على الأقل</li>
                    ...
                    <li className={`flex justify-start transition-all ${validations.hasSymbol ? 'line-through text-[#009e5a]' : ''}`}>رمز واحد على الأقل (% $ # @ & / - _)</li>
                  </ul>
                </div>
                    ...
                    {password && !passwordError && (
                      <div className="mt-3">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-[11px] text-[#666]">قوة كلمة السر:</span>
                          <span className="text-[11px] font-bold" style={{ color: passwordStrength.color }}>{passwordStrength.text}</span>
                        </div>
